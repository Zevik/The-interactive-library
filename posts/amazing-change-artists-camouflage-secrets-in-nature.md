...
<style>
    ...
    .octopus.coral {
     ...
     /* Animation hint: pulse slightly on color change */
    .octopus.changing {
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const environmentButtons = document.querySelectorAll('.environment-selector button');
        const simulationArea = document.querySelector('.simulation-area');
        const toggleButton = document.getElementById('toggle-explanation');
        const explanationDiv = document.getElementById('explanation');

        // Function to change octopus and environment
        function setEnvironment(environment) {
            // Add 'changing' class to trigger animation start
            octopus.classList.add('changing');

            // Wait a moment to allow the animation to start before changing colors
            setTimeout(() => {
                 // Remove existing environment classes from octopus
                octopus.classList.remove('rocky', 'sandy', 'coral', 'openwater');
                 // Remove existing environment classes from simulation area
                simulationArea.classList.remove('rocky-env', 'sandy-env', 'coral-env', 'openwater-env');

                // Add the selected environment class to octopus and simulation area
                octopus.classList.add(environment);
                simulationArea.classList.add(environment + '-env');

                // Remove the 'changing' class after the transition should be mostly done
                // The transition duration for color/image is 1s, so 900ms is good
                setTimeout(() => {
                     octopus.classList.remove('changing');
                }, 900);

            }, 100); // Short delay to ensure 'changing' class is applied and transition starts


            // Highlight active button
            environmentButtons.forEach(btn => {
                btn.classList.remove('active');
                if (btn.dataset.environment === environment) {
                    btn.classList.add('active');
                }
            });
        }

        // Add event listeners to environment buttons
        ...
        // Add event listener to the explanation toggle button
        toggleButton.addEventListener('click', () => {
            if (explanationDiv.style.display === 'none') {
                explanationDiv.style.display = 'block';
                toggleButton.textContent = 'הסתרת ההסבר המלא';
                toggleButton.style.backgroundColor = '#ef476f'; /* Muted Pink/Red */
                toggleButton.style.boxShadow = '0 6px 8px rgba(0, 0, 0, 0.15)';
            } else {
                explanationDiv.style.display = 'none';
                toggleButton.textContent = 'נחשפים הסודות: לחצו להסבר המלא';
                 toggleButton.style.backgroundColor = '#06d6a0'; /* Medium Sea Green */
                 toggleButton.style.boxShadow = '0 6px 8px rgba(0, 0, 0, 0.15)';
            }
        });

         // Set initial environment and active button (matches the default HTML class)
