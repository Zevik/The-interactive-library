...
<div id="full-explanation" style="display: none;">
    ...
    <h3>עקרונות הפענוח: צעד אחר צעד</h3>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const symbolGroups = document.querySelectorAll('.symbol-group');
        ...
        symbolGroups.forEach(group => {
            group.addEventListener('click', (event) => {
                ...
                const clues = JSON.parse(group.dataset.clues);
                const parentDecipherArea = event.target.closest('.decipher-area');
                const cluesBox = parentDecipherArea.querySelector('.clues-box');

                // Clear previous clues animation class
                cluesBox.classList.remove('fadeIn');
                // Force reflow to restart animation
                void cluesBox.offsetWidth;

                if (clues.length > 0) {
                    cluesBox.innerHTML = '<strong>רמזים לפענוח:</strong> ' + clues.join(' | ');
                } else {
                     cluesBox.innerHTML = '<em>אין רמזים זמינים עבור סימן זה כרגע. נסו סימן אחר.</em>';
                }

                // Add fade-in animation class
                cluesBox.classList.add('fadeIn');
            });
        });
        ...
        toggleButton.addEventListener('click', () => {
            ...
            if (isHidden) {
                ...
                // Remove and add fadeIn class to re-trigger animation
                explanationDiv.classList.remove('fadeIn');
                void explanationDiv.offsetWidth; // Force reflow
                explanationDiv.classList.add('fadeIn');
                toggleButton.textContent = 'הסתרת הסיפור המלא';
            } else {
                explanationDiv.style.display = 'none';
                toggleButton.textContent = 'הצגת הסיפור המלא: איך באמת פענחו את הכתבים?';
            }
        });

        // Initial state of clues boxes
         document.querySelectorAll('.clues-box').forEach(box => {
             box.innerHTML = 'לחצו על הסימנים כדי לגלות רמזים לפענוח...';
         });
