...
/* Step 4: Wax Removed (Wax layer disappears, base color is blue, areas previously waxed show original color) */
...
document.addEventListener('DOMContentLoaded', () => {
    const canvas = document.getElementById('batik-canvas');
    const nextButton = document.getElementById('next-batik-step');
    ...
    // Define steps - engaging descriptions and corresponding CSS classes
    const steps = [
        { desc: "המסע מתחיל: הבד הלבן והטהור מוכן לקסם.", class: 'batik-step-0', buttonText: "ציירו את הדוגמה הראשונה" },
        { desc: "צעד ראשון: קווי המתאר העדינים מופיעים על הבד.", class: 'batik-step-1', buttonText: "מרחו שכבת שעווה קסומה" },
        { desc: "מסך השעווה: שעווה חמה מגנה על האזורים שאינם אמורים לקבל את הצבע הראשון.", class: 'batik-step-2', buttonText: "טבלו באמבט הצבע הכחול" },
        { desc: "צבע ראשון: הבד עוטף את עצמו בכחול עמוק. האזורים המכוסים בשעווה מתנגדים באומץ!", class: 'batik-step-3', buttonText: "הסירו את השעווה וחשפו את הנסתר" },
        { desc: "הקסם נחשף: השעווה נעלמת, והאזורים שהיו מוגנים מתגלים בצבעם הלבן המקורי. נוצרה הדוגמה הראשונה!", class: 'batik-step-4', buttonText: "הוסיפו שכבת שעווה נוספת לדוגמה הבאה" },
        { desc: "שכבה נוספת: שעווה נמרחת שוב, הפעם כדי להגן גם על חלק מהאזורים הכחולים והלבנים שכבר נוצרו.", class: 'batik-step-5', buttonText: "טבלו באמבט הצבע הירוק" },
        { desc: "צבע שני: הבד נטבל בירוק. רק האזורים שלא כוסו בשעווה מקבלים את הצבע החדש.", class: 'batik-step-6', buttonText: "הסירו את כל השעווה!" },
         { desc: "היצירה הושלמה: כל השעווה הוסרה. הדוגמה הרב-צבעונית והמורכבת נחשפת במלואה! בד הבאטיק מוכן.", class: 'batik-step-7', buttonText: "התחילו מסע באטיק חדש" },
        // For simplicity, stop after two color cycles. The explanation mentions repeating.
    ];

    // Function to update the display based on the current step
    function updateDisplay() {
        // Remove all step classes
        steps.forEach(step => canvas.classList.remove(step.class));
        // Add the class for the current step
        canvas.classList.add(steps[currentStep].class);
        // Update the description text
        stepDescription.textContent = steps[currentStep].desc;

        // Update button text based on the *next* step's intended action
        const nextStepIndex = (currentStep + 1) % steps.length;
        nextButton.textContent = steps[nextStepIndex].buttonText;
    }
    ...
    // Initialize the display to step 0 on page load
    updateDisplay();
