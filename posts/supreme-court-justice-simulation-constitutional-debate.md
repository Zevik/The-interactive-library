...
<div id="simulation-app">
                ...
                <div class="argument respondent">
                    <h4><i class="icon-government"></i> המשיבה (המדינה):</h4>
                     <ul>
                        ...
                        <li><i class="icon-arrow-right"></i> החוק עומד במבחני המידתיות: הוא משרת מטרה ראויה (הגנה על קטינים וסדר ציבורי), קיים קשר רציונלי בין האיסור למטרה, והפגיעה אינה עולה על הנדרש לאור חומרת הסיכון הפוטנציאלי, היקף ההגבלה המצומצם יחסית לכלל המדינה, והעדר חלופות יעילות מספיק.</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <section id="initial-stance" class="card" style="display:none;">
        <div class="card-header">
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const initialStanceSection = document.getElementById('initial-stance');
        const initialStanceTextarea = document.getElementById('initial-stance-text');
        const confirmStanceButton = document.getElementById('confirm-stance');
        ...
        const finalReasoningText = document.getElementById('final-reasoning-text');
        const simulatedOutcomeDiv = document.getElementById('simulated-outcome');
        const showExplanationButton = document.getElementById('show-explanation-button');
        const explanationDiv = document.getElementById('explanation');
        const stepsLeftSpan = document.getElementById('steps-left');


        let discussionStep = 0;
        const maxDiscussionSteps = 4; // Increased steps for deeper interaction

        const responses = {
            'ask-petitioner-1': {
                text: "העותר משיב: אכן. הצענו לממשלה ולכנסת לבחון חלופות ממוקדות יותר, למשל, הגבלות מחמירות רק על הפגנות הכוללות רעש חריג או אלימות, או הגבלת מרחק מצומצמת בהרבה. חלופות אלו נדחו ללא נימוק מספק לטעמנו, מה שמחזק את הטענה שהחוק אינו האמצעי שפוגע הכי פחות.",
                speaker: 'petitioner'
            },
            'ask-petitioner-2': {
                text: "העותר משיב: בית המשפט צריך לאזן תוך מתן משקל בכורה לזכות היסוד, אלא אם כן הוכח מעל לכל ספק סביר שפגיעה בה הכרחית למניעת סיכון קונקרטי ומוחשי. במקרה זה, ההגבלה גורפת ואינה מבוססת על הערכת סיכון ספציפית לכל מוסד, אלא על חשש כללי ומוגזם.",
                speaker: 'petitioner'
            },
            'ask-respondent-1': {
                text: "המשיבה משיבה: קביעת המרחק (150 מטרים) והזמנים התבססה על חוות דעת כלליות של כוחות הביטחון ומשרד החינוך לגבי האזורים שבהם הפגנות עלולות לגרום להפרעה ניכרת וחרדה בסמוך למוסדות חינוך. אין מדובר בנתונים מדויקים מכל אתר ואתר, אך ההערכה היא שהמרחק והזמנים הללו נחוצים ליצירת 'אזור חיץ' אפקטיבי.",
                speaker: 'respondent'
            },
            'ask-respondent-2': {
                text: "המשיבה משיבה: נשקלו אמצעים אחרים, אולם הגיעו למסקנה שרק איסור גורף במרחק וזמנים מוגדרים יוכל להבטיח באופן אמין את שלומם ושגרת יומם של הילדים. אמצעים כמו הגבלת רעש או נוכחות משטרתית נחשבו לקשים יותר לאכיפה יעילה ולפוגעניים פחות אך גם פחות אפקטיביים בהשגת המטרה של יצירת סביבה רגועה לילדים.",
                speaker: 'respondent'
            },
            'respond-judgeB-1': {
                text: "'מסכים עם שופט ב', האיזון בין זכות ההפגנה החשובה לבין הצורך להגן על קטינים הוא אכן ליבת הדיון. עלינו לבחון היטב האם החוק הנוכחי משיג את האיזון הנכון על פי עקרונות המידתיות.'<br><strong>שופט ב':</strong> 'בהחלט. הנטל עלינו לוודא שהמחוקק לא חרג ממתחם הסבירות החוקתית באיזון הזה.'",
                speaker: 'user'
            },
            'respond-judgeA-1': {
                text: "'כמו שופטת א', גם אני מוטרד מהרוחב הגורף של ההגבלה הזו. איסור גורף על כל התקהלות של מעל 20 איש במרחק כזה, ללא קשר לאופייה, נראה כפוגעני יתר על המידה בזכות היסוד. נצטרך לבדוק אם פגיעה כה רחבה עומדת במבחן המידתיות.'<br><strong>שופטת א':</strong> 'אני שמחה שאתה שותף לדאגה זו. נדרשת בדיקה קפדנית של מבחן הקשר הרציונלי והאמצעי שפוגע פחות.'",
                 speaker: 'user'
            },
             // Follow-up responses from AI judges after user action
             'follow-up-judgeA-1': { text: "<strong>שופטת א':</strong> 'האם יש לחבריי שאלות נוספות לצדדים לאור התשובות שקיבלנו?'", speaker: 'judge-a'},
             'follow-up-judgeB-1': { text: "<strong>שופט ב':</strong> 'שמענו את העותרים והמשיבים, ואת ההתייחסות שלכם. האם טיעונים אלו סייעו לכם לגבש עמדה ראשונית לגבי חוקתיות החוק?'", speaker: 'judge-b'},
